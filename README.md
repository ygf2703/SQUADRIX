...
**SQUADRIX** נפתחת כמערכת נקייה וניטרלית: שם המועדון, הלוגו, צבעי הממשק ומקור הנתונים מוזנים רק על ידי מנהל המערכת דרך מסך ההגדרות.
...
- `202608010007_club_branding.sql`
   ...
   למאגר שכבר הוגדר עם נתוני הדגמה, הריצו פעם אחת גם את `202608020001_reset_demo_data.sql`. היא מוחקת סגל, משחקים, אימונים, סטטיסטיקות, לוגו פעיל וקישורי מקור קיימים, אך שומרת על המשתמשים ועל ההרשאות. קובץ לוגו ישן, אם קיים, יישאר ב־Storage אך לא יהיה מחובר למועדון.
...
קיימים גם קובצי בדיקה אופציונליים (אינם נטענים אוטומטית):
