...
export const playersService = {
  async list(): Promise<Player[]> { if (!supabase) return []; const team = await teamSelection.getCurrent(); const { data, error } = await supabase.from('players').select('*,player_seasons!inner(*,player_season_statistics(*))').eq('player_seasons.team_id', team.id).order('full_name'); if (error) throw error; return data as Player[]; },
  async create(input: CreatePlayerInput) {
    if (!supabase) throw new Error('Supabase is not configured');
    const team = await teamSelection.getCurrent();
    if (!team.current_season_id) throw new Error('No active team and season were found.');
