...
export const scheduleService = {
  ...
  async create(input: ScheduleEventInput) { if (!supabase) throw new Error('Supabase is not configured'); const { data: team, error: teamError } = await supabase.from('teams').select('id,current_season_id').not('current_season_id', 'is', null).limit(1).single(); if (teamError || !team.current_season_id) throw new Error('No active team and season were found.'); const { error } = await supabase.from('team_schedule_events').insert({ ...input, team_id: team.id, season_id: team.current_season_id, ends_at: input.ends_at || null }); if (error) throw error; },
