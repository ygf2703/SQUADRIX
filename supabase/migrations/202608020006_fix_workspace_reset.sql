...
begin
  ...
  delete from public.match_player_stats where true;
  if to_regclass('public.match_lineups') is not null then
    execute 'delete from public.match_lineups where true';
  end if;
  delete from public.matches where true;
  delete from public.team_schedule_events where true;
  if to_regclass('public.player_season_statistics') is not null then
    execute 'delete from public.player_season_statistics where true';
  end if;
  delete from public.player_seasons where true;
